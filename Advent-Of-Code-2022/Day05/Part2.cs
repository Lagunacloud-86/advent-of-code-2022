...
public sealed class Part2 : IAOCProject<string>
{
    ...
    {
        Span<char> queue = stackalloc char[instruction.HowMany];
        for (var moveCount = 0; moveCount < instruction.HowMany; moveCount++)
            queue[moveCount] = _cargo[instruction.FromId].Pop();
        
        for (var moveCount = instruction.HowMany - 1; moveCount >= 0; moveCount--)
            _cargo[instruction.ToId].Push(queue[moveCount]);
    }
