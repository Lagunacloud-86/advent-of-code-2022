...
public sealed class Part2 : IAOCProject<int>
{
    ...
    public int Run()
    {
        ...
        {
            if (HasOverlappingWork(SectionList[i]))
                score++;
        }
    ...
    {
        var length = Math.Abs(item.Section2.Start - item.Section1.End + item.Section2.End - item.Section1.Start);
        return (item.Section1.Length + item.Section2.Length) > length;
    }
